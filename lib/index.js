...
var buildPackage = function(obj, callback) {
  ...
  var tempDir = path.join(os.tmpdir(), 'scorm-packager');
  ...
  var rootDir = path.dirname(fs.realpathSync(__filename)),
    definitionFileList = files(
      ...
      return {
        ...
        destination: path.join(tempDir, file)
      };
    });
  ...
  fse.outputFile(
    ...
    function(err) {
      if (err) {
        ...
        _logSuccess("create " + path.join(tempDir, "imsmanifest.xml"));
        fse.outputFile(
          path.join(tempDir, "metadata.xml"),
          metadata(obj),
          function(err) {
            if (err) {
              return _logError(err);
            } else {
              _logSuccess("create " + path.join(tempDir, "metadata.xml"));
